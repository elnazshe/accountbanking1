...
public interface LegalCostumerDao extends JpaRepository<LegalCostumer, Integer> {
    ...
    @Query(" select c from LegalCostumer c  where (c.companyNumber=:companyNumber) and (c.isDeleted=FALS) ")
    ...
    @Query(" select c from LegalCostumer c where (c.name like %:name% or :name is null) or " +
            " (c.companyTpe=:companyType or :companyType is null) and (c.isDeleted=FALS)")
